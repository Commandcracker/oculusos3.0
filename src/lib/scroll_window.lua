...
-- by SquidDev
-- MIT License
-- GitHub: https://github.com/SquidDev-CC/mbs/blob/c0dd46832610ec78b817184056e1b80b5c5da964/lib/scroll_window.lua
-- Last updated: Oct 3 2021

-- Changelog:
-- [Date]        [Author]         [Change]
-- (July 24 2022) (Commandcracker) (export create)
...
local function create(original)
    ...
    function redirect.write(writeText)
        ...
        text_colour[scroll_cursor_y] = sub(lineColor, preStart, preStop) ..
            rep(cur_text_colour, #writeText) .. sub(lineColor, postStart, postStop)
        back_colour[scroll_cursor_y] = sub(lineBack, preStart, preStop) ..
            rep(cur_back_colour, #writeText) .. sub(lineBack, postStart, postStop)
        cursor_x = pos + #writeText
    end
    ...
    function redirect.blit(writeText, writeFore, writeBack)
        ...
        text[scroll_cursor_y] = sub(lineText, preStart, preStop) .. writeText .. sub(lineText, postStart, postStop)
        text_colour[scroll_cursor_y] = sub(lineColor, preStart, preStop) ..
            writeFore .. sub(lineColor, postStart, postStop)
        back_colour[scroll_cursor_y] = sub(lineBack, preStart, preStop) ..
            writeBack .. sub(lineBack, postStart, postStop)
        cursor_x = pos + #writeText
    end
