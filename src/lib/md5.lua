...
-- Changelog:
-- [Date]        [Author]         [Change]
-- (July 13 2022) (Commandcracker) (made mt/ByteArray a sperate library)
...
local function digestblock(m, C)
    ...
    for j = 0, 63 do
        ...
        local dTemp = d
        a, b, c, d = dTemp,
            (b + lrotate((a + f + K[j + 1] + m[g + 1]) % mod32, s[bor(blshift(r, 2), band(j, 3)) + 1])) %
            mod32, b, c
    end
    C[1] = (C[1] + a) % mod32
    C[2] = (C[2] + b) % mod32
...
local function toBytes(t, n)
    ...
    return setmetatable(b, ByteArray)
end
